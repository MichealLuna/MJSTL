...
enum class format{
    ...
    bold        = 1, //Bold or increased off.
    faint       = 2, //Faint(decreased intensity).
    italic      = 3, //Italic font.
...
//Manages associateed stream buffer.
...
inline bool is_modifialbe()
{
#if defined(REDBUD_LINUX) || defined(REDBUD_OSX)
    static constexpr const char* terms[] = {
        "ansi","color","console","cygwin","gnome","knosole",
...
//Sets the color and format of the text.
#if defined(REDBUD_LINUX) || defined(REDBUD_OSX)
template<typename T>
...
{
    state &= 0xFFF0;
    state |= (static_cast<WORD>(0x8) | get_rgb(static_cast<WORD>(color) - 90));
}
